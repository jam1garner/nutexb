use tegra_swizzle::{
    ...
    swizzle_block_linear, swizzled_surface_size,
};
...
pub fn swizzle_mipmaps_to_data(
    ...
    for (i, mip) in mipmaps.into_iter().enumerate() {
        let mip_width = std::cmp::max(div_round_up(width >> i, block_width), 1);
        let mip_height = std::cmp::max(div_round_up(height >> i, block_height), 1);
